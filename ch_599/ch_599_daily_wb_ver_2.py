...
p, residual, rank, singular_values, conditioning = np.polyfit(x_cal, y_cal, 1, full=True)

print residual
# raise SystemExit(0)
a_stage = polyfit(x_cal, y_cal, 1)
print a_stage['determination']
...
plt.xlim([(min(x_cal)-5), (max(x_cal)+5)])
plt.ylim([0, 3.5])
plt.xlabel(r'\textbf{Capacitance} (ohm)')
plt.ylabel(r'\textbf{Stage} (m)')
plt.legend(loc='upper left')
plt.title(r'Capacitance Sensor Calibration for 599 Check dam')
plt.text(x=1765, y=275, fontsize=15, s=r"\textbf{{$ y = {0:.1f} x  {1:.1f} $}}".format(coeff_cal[0], coeff_cal[1]))
plt.savefig('/media/kiruba/New Volume/ACCUWA_Data/python_plots/check_dam_evap/sensor_calib_599')
# plt.show()
...
def read_correct_ch_dam_data(csv_file):
    ...
    water_level['calibrated value'] /= 1000.0  # convert to metre
    water_level['calibrated value'] = myround(water_level['calibrated value'], decimals=2)
    ...
    water_level.drop(['scan no', 'date', 'time', 'date_time'], inplace=True, axis=1)  # 'raw value',

    return water_level


## Read check dam data
block_1 = '/media/kiruba/New Volume/ACCUWA_Data/check_dam_water_level/2665_005_001.CSV'
water_level_1 = read_correct_ch_dam_data(block_1)
block_2 = '/media/kiruba/New Volume/ACCUWA_Data/check_dam_water_level/2665_003_008.CSV'
water_level_2 = read_correct_ch_dam_data(block_2)
block_4 = '/media/kiruba/New Volume/ACCUWA_Data/check_dam_water_level/2665_005_002.CSV'
...
# raise SystemExit(0)
for index in water_level.index:
    if water_level['stage(m)'][index.strftime(date_format)] < 0:
        water_level['stage(m)'][index.strftime(date_format)] = 0.0
# print water_level.head()
# print water_level.tail()
fig = plt.figure()
plt.plot(x_cal, y_cal/1000.0, 'b-o', label=r'Observation')
...
weather_df.to_csv('/media/kiruba/New Volume/ACCUWA_Data/Checkdam_water_balance/ch_599/weather.csv')
...
water_balance_df['overflow(cu.m)'] = 0.00
...
print min(water_balance_df.index)
...
for index, row in water_balance_df.iterrows():
    if index > min(water_balance_df.index):
        ...
        e1 = water_balance_df['Evaporation (cu.m)'][index.strftime(date_format)]
        if ((v1 -v2) > (14 + e1)) and obs_stage < 1.9:
            water_balance_df['pumping status'][index.strftime(date_format)] = 1

"""
Daily Totals of Rain, Evaporation, Overflow
"""
sum_df = water_balance_df[['Rain Collection (mm)', 'Evaporation (cu.m)', 'Evaporation (mm/30min)', 'pumping status', 'overflow(cu.m)']]
...
for index in water_balance_daily_df.index:
    if index > initial_time_stamp:
        ...
        if (sum_df == 0) and (water_balance_daily_df['change_storage(cu.m)'][index.strftime(daily_format)] < 0) and (water_balance_daily_df['overflow(cu.m)'][index.strftime(daily_format)] == 0) and (water_balance_daily_df['pumping status'][index.strftime(daily_format)] == 0):
...
plt.title(r"No inflow day's stage - infiltration relationship for 599 check dam")
plt.text(x=0.4, y=.003, fontsize=15, s=r'$Infiltration = {0:.2f}{{h_{{av}}}}^{{{1:.2f}}}$'.format(popt[0], popt[1]))
plt.savefig('/media/kiruba/New Volume/ACCUWA_Data/python_plots/check_dam_599/new_stage_inf_exp_dry_599_30min')
"""
...
plt.title(r"Inflow day's stage - infiltration relationship for 599 check dam")
plt.savefig('/media/kiruba/New Volume/ACCUWA_Data/python_plots/check_dam_599/new_rain_inf_599_30min')
